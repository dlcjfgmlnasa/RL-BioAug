...
# Dataset
dataset_name: "sleep-edfx"          # or "chb-mit"
data_dir: "./data/sleep-edfx/"
split_ratio: 0.8
batch_size: 1024
num_workers: 4
search_data_ratio: 0.1              # % of data for Phase 1
...
# Actions & Sampling
num_actions: 5
top_k: 1                            # Deterministic selection

# Training
epochs: 300                         # Phase 1 epochs
retrain_epochs: 50                  # Phase 2 epochs
lr_enc: 0.0001
lr_agent: 0.0001
...
|--------|---------|-------------|
| `sleep_edf_config_topk_1.yaml` | Sleep-EDFX | Top-1 action selection |
| `sleep_edf_config_topk_3.yaml` | Sleep-EDFX | Top-3 action selection |
| `sleep_edf_config_topk_5.yaml` | Sleep-EDFX | Top-5 action selection |
| `ptb_xl_config.yaml` | PTB-XL | ECG dataset config |
| `sleep_edf_linear_probing.yaml` | Sleep-EDFX | Linear probing evaluation |
...
- 1D convolutional ResNet for EEG signals
- Input: `(Batch, 1, Time_steps)`
- Architecture: 4 residual blocks (64 → 128 → 256 → 512 channels)
- Output: Encoder features (`h`) and projected features (`z`, dim=256)

### Policy Network: ContextAgent

- Transformer-based architecture
- Input: State sequence, previous actions, previous rewards
- Components:
  - State/action/reward embeddings
  - Positional encoding + Transformer encoder
  - Policy head for action probabilities
- Default config: 128 hidden dim, 4 attention heads, 2 layers
